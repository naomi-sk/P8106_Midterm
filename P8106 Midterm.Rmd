...
* Some of the predictors have estimated degrees of freedom (edf) greater than 1 — specifically height, BMI, SBP, and time — indicating that they are nonlinear smooth functions. This is supported by the GAM predictor plots. Weight, while slightly above 1, was not statistically significant and does not show strong evidence of nonlinearity.

* LDL had an EDF < 1 (EDF = 0.55), indicating that the term was heavily penalized and shrunk toward zero. This indicates LDL contributed very little to the model, with a negligible effect on the outcome. The corresponding predictor plot supports this interpretation.

* Among the nonlinear terms, SBP, BMI, time, and height are significant (p < 0.05), suggesting meaningful nonlinear associations with the log antibody levels.
...
MARS has the smallest RMSE, so we will try to apply this model to dat2. 

# Test Error for Selected Model Applied to New Dataset: MARS

```{r}
## NOTE: Naomi commented out this code because it kept running errors: ie y2 hasnt been defined
set.seed(299)
...
set.seed(299)
#mars_rmse <- sqrt(mean((y2 - mars.pred)^2))
#mars_rmse
...
#predy2_pcr2 <- predict(mars.fit, newdata = dat2)
...
y_testing <- dat2$log_antibody

# Predict on test data using trained MARS model
y_pred_MARS <- predict(mars.fit, newdata = dat2)
